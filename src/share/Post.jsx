...
const Post = (props) => {
  ...
  const userImg = props.user.profileImg;
  const postText = { props.post.text ? props.post.text : "옷을 인생을 그러므로 없으면 것은 이상은 것은 우리의 위하여, 뿐이다. 이상의 청춘의 뼈 따뜻한 그들의 그와
            약동하다. 대고, 못할 넣는 풍부하게 뛰노는 인생의 힘있다."};
  const postImg = { props.post.img ? props.post.img : "https://images.unsplash.com/photo-1605001011156-cbf0b0f67a51?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1374&q=80"};
  const uploadDate = { props.post.date ? props.post.date : "2099년 13월 41일"};
  const numOfLike = { props.post.like };
  const numOfCommennt = { props.post.comments.length ? props.post.comments.length : 0 };
        ...
        <div className="img_and_icons">
          ...
          <div className="flex flex-row mb-[16px]">
            <button type="button" className="flex items-center mr-[16px] text-[#767676]">
              ...
              <span className="text-[12px] leading-[12px] ml-[6px]">{numOfLike}</span>
            </button>
            <button type="button" className="flex items-center mr-[16px] text-[#767676]">
              <img className="w-[20px] h-[20px]" src={commentImg} alt="댓글 확인하기" />
              <span className="text-[12px] leading-[12px] ml-[6px]">{numOfCommennt}</span>
            </button>
          </div>
        </div>
        <span className="text-[10px] font-normal leading-[12px] text-[#767676]">{uploadDate}</span>
      </div>
    </section>
  );
};

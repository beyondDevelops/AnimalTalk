...
// Home
export const getFollowersFeeds = async (feedParam = 0, token, options = {}) => {
  const res = await api.get(
    `/post/feed?limit=10&skip=${feedParam * 10}`,
    {
      headers: {
...
export const getPageOwnerFeeds = async (pageAccount, feedParam = 0, token, options = {}) => {
  const res = await api.get(
    `/post/${pageAccount}/userpost/?limit=10&skip=${feedParam * 10}`,
    {
      headers: {
        Authorization: `Bearer ${token}`,
      },
